...
class StandupForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.participation.completed:
            answers = dict([
                (a.question, a.answer) for a in self.participation.answers.all()
            ])

        questions = participation.standup.event.standup_type.questions.all()
        for question in questions:
            field_name = 'question_%d' % question.id
            self.fields[field_name] = forms.CharField(
                required=False,
                label=question.question,
                initial=answers.get(question),
                widget=forms.Textarea()
            )
            if not question.important:
                self.fields[field_name].help_text = (
                    'This field is optional, '
                    'please leave it empty if it does not apply!'
                )

            if (
                question.prefill_last_answer
                and prev_parti
                and not self.participation.completed
            ):
                lastans = (
                    prev_parti.answers
                    .filter(question=question.prefill_last_answer)
                    .first()
                )
                if lastans:
    ...
    def save(self):
        questions = (
            self.participation.standup.event.standup_type.questions.all()
        )
        for question in questions:
            field_name = 'question_%d' % question.id
            ans, created = self.participation.answers.get_or_create(
                question=question,
                defaults={'answer': self.cleaned_data[field_name]}
            )
            if not created:
